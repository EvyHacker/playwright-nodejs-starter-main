...
const emailCreat = `p2p_${f_firstName}.${f_lastName}_dogRun@engagingnetworks.online`.toLowerCase();
const emailCreateNewTeam = `p2p_${f_firstName}.${f_lastName}_CreateNewTeamDogRun@engagingnetworks.online`.toLowerCase();
const emailCreateJoin = `p2p_${f_firstName}.${f_lastName}_CreateJoinDogRun@engagingnetworks.online`.toLowerCase();
...
const newTeam = ('DogRun ' + f_firstName + ' ' + formattedDate)

test('Registration Dog Run Individual', async ({ page }) => {
...
test.slow()
  ...
  await page.getByText('Run 5k With Your Dog').click();
...
test('Registration Dog Run No Donations', async ({ page }) => {
...
test('Registration Dog Run Join the Team No Donations', async ({ page }) => {
  ...
  await page.getByPlaceholder('Search for name, team').fill('DogRun Irving 2024-03-14');
...
test.skip('Validation of Dog run', async ({ page }) => {
